...
impl DynAomiTool for GetTrades {
    ...
    fn run(app: &HooditApp, args: TradesArgs, _: DynToolCallCtx) -> Result<Value, String> {
        ...
        let mut trades = response_rows(&response)
            .into_iter()
        ...
        Ok(model::ok(
            json!({"token":token,"pool":{"pool_id":selected_pool["pool_id"],"dex_id":selected_pool["dex_id"],"dex_name":selected_pool["dex_name"]},"trades":trades,"coverage":{"scope":"single_pool","lookback_seconds":86400,"provider_trade_cap":300,"returned":returned,"complete_history":false}}),
            read.sources,
            read.warnings,
        ))
    }
