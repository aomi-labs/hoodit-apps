# Hoodit v1.1.0 validation record

Validated locally on 2026-09-17. This record separates contract and build
checks from live provider reads. No wallet transaction was prepared, signed,
broadcast, or simulated by these checks.
...
- `cargo test --workspace --locked` passed 18 unit tests and three contract
...
- The SDK 5.1.0 compile and manifest checks passed. The manifest contains
  exactly seven tool descriptors, two app skills, and two optional secret
  slots. A locked standalone build of `apps/hoodit` outside the workspace
  passed.
...
The independent validator's `--implementation-fixtures DIR` mode validated 25
...
holding, unknown-decimals partial output, and typed missing-provider and invalid
argument errors. The deterministic
...
envelope passed the canonical v1.1.0 schemas. An additional exact holding read
for a nonzero WETH balance returned a `quoted` valuation through a KyberSwap
route. These checks requested data and quotes only; they did not prepare,
authorize, sign, or broadcast a transaction.
...
Run the reusable smoke only after application `2937810` is active in staging:
...
error bodies and action payloads are never printed. The script has not been run
against the new deployment yet.

No Hoodit v1.1.0 deployment is claimed by this record.
